import "./FeedSettingChangePassword.css";

function FeedSettingChangePassword() {
  return (
    <>
      <div className="feedsettings__changepassword">
        <div className="flex flex-col items-center xl:gap-y-20 lg:gap-y-6 sm:gap-y-4 xs:gap-y-6  xl:items-center xl:pt-14 lg:pt-4 md:pt-8 sm:pt-10 sm:justify-around xs:pt-10 ">
          <h1>Change Password</h1>
        </div>
      </div>
    </>
  );
}

export default FeedSettingChangePassword;
